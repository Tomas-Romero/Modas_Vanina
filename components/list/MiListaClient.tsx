    ...
    <div className="mx-auto max-w-2xl px-4 py-8 md:px-8 md:py-12">
      ...
      {items.length === 0 ? (
        <div className="mt-10 flex flex-col items-center rounded-2xl border border-dashed border-line py-16 text-center">
          <LogoMark size="sm" className="opacity-50" />
          <ShoppingBag className="mt-6 h-8 w-8 text-ink-soft" />
        ...
        <>
          ...
          <div className="mt-8 flex flex-col gap-3">
            <a
              href={buildListWhatsAppLink(items)}
              target="_blank"
              rel="noopener noreferrer"
              onClick={() => items.forEach((item) => recordWhatsAppClickAction(item.id))}
            >
